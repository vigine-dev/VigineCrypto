...
std::vector<std::byte> hkdfSha256(std::span<const std::byte> secret,
                                  std::span<const std::byte> salt, std::span<const std::byte> info,
                                  std::size_t outLength)
{
    ...
    if (!salt.empty())
        params[index++] = OSSL_PARAM_construct_octet_string(
            OSSL_KDF_PARAM_SALT, const_cast<std::byte *>(salt.data()), salt.size());
    if (!info.empty())
        params[index++] = OSSL_PARAM_construct_octet_string(
            OSSL_KDF_PARAM_INFO, const_cast<std::byte *>(info.data()), info.size());
    ...
    if (EVP_KDF_derive(ctx.get(), reinterpret_cast<unsigned char *>(out.data()), outLength,
                       params) != 1)
        return {};
    return out;
}
